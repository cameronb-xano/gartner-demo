query "customer-data/{claim_id}" verb=GET {
  api_group = "policy"
  description = "Snowflake-backed customer and policy context for a claim. Returns deterministic demo data if Snowflake is not configured."
  input {
    int claim_id
  }
  stack {
    ...
    try_catch {
      try {
        ...
        debug.log { value = "Policy Data Snowflake lookup unavailable; returning demo fallback data." }
      }
    }
  }
  response = $context
  guid = "YPlJ3LEX8lmjTH_ngOKhMIcnZ1c"
}
