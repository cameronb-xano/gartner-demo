query "claims" verb=GET {
  api_group = "policy"
  description = "List claim facts from Snowflake for the recorded claims frontend."
  input {
  ...
  stack {
    db.external.snowflake.direct_query {
      sql = "select claim_id, claim_number, customer_id, claim_type, claim_status, claim_priority, assigned_queue, amount_requested, summary, opened_at from XANO_DEMO.CLAIMS_DEMO.CLAIM_POLICY_CONTEXT where amount_requested is not null order by claim_id desc limit {{ $input.per_page|sql_esc }}"
  ...
  guid = "HO5QhRER00_GHMxYT0AsPA0vp0Y"
}
